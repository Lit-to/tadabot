...
class Answer(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        return_message, em, v = check_answer(
            self.title, self.answer.value, interaction.user.mention
        )
...
class Question(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="謎解きフォーム", timeout=None)
...
def setbutton(title: str):
    button = discord.ui.Button(
        label=title + "にこたえる！", style=discord.ButtonStyle.success, custom_id=title
    )
...
class QuestionAdd(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="謎解き追加フォーム", timeout=None)
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        data, ids = nazo.open_file()
        suc, data, ids = nazo.add_contents(
            self.question.value, self.answer.value, data, ids
        )
        if suc:
            nazo.write_file(data)
            await interaction.response.send_message(
                "", view=setbutton(self.question.value), ephemeral=False
            )
        else:
            await interaction.response.send_message(
                "タイトルが被っています", ephemeral=True
            )
...
async def on_button_click(interaction: discord.Interaction):
    fo.printf(
        interaction.user.name, 'did "Interaction":', interaction.data["custom_id"]
    )
    ...
    if custom_id[0] == "B":
        ...
        await interaction.followup.edit_message(
            message_id=interaction.message.id, view=bingo_button(True)
        )
        ...
        title = custom_id[1:]
        await interaction.response.send_message(
            title + "の答えは:**" + getAnswer(title) + "**でした！！", ephemeral=True
        )
    elif custom_id[0] == "?":
        await interaction.response.send_message(
            "## 答え出すけどいい？\nもし、いらない場合は右下の これらのメッセージを削除するを押してね。↓",
            view=setanswer(custom_id[1:], True),
            ephemeral=True,
        )
    else:
        c = nazo.open_file()
...
@app_commands.describe(
    command="add / remove / check / set", title="タイトル", answer="答え"
)
async def answer(
    ...
    if command == "add":
        if title == "" or answer == "":
            ...
            if suc:
                nazo.write_file(data)
                await interaction.response.send_message(
                    "", view=setbutton(title), ephemeral=False
                )
            else:
                await interaction.response.send_message(
                    "タイトルが被っています", ephemeral=True
        ...
        if title == "":
            ...
            if suc:
                ...
                await interaction.response.send_message(
                    "1..2の...ポカン！**"
                    + title
                    + "**というタイトルの謎解きの答えをきれいさっぱり忘れたよ！",
                    ephemeral=True,
                )
            else:
                await interaction.response.send_message(
                    "タイトルが見つかりませんでした、すまん", ephemeral=True
                )
    elif command == "check":
        if title == "" or answer == "":
            ...
            return_message, em, v = check_answer(
                title, answer, interaction.user.mention
            )
            await interaction.response.send_message(
                return_message, ephemeral=em, view=v
            )
    else:
...
@tree.command(
    name="lits", description="指定回数(り・と・)って言うよ、自分にしか見えないよ"
)
...
@tree.command(
    name="choice", description="自分と同じ通話に居る人から一人メンションするよ"
)
async def test(interaction: discord.Interaction):
...
async def notice(interaction: discord.Interaction, operation: str = "card"):
    if operation == "card":
        ...
        await interaction.response.send_message(
            "現在のビンゴの数は" + "".join(result), ephemeral=False
        )
    elif operation == "start":
        await interaction.response.send_message(
            "ボタンを押して開始！間違えた場合はこっそりメッセージを消してください。",
            view=bingo_button(),
            ephemeral=True,
        )
    else:
...
async def exits(interaction: discord.Interaction):
    ...
    if interaction.user.id == int(379155307546542081):
        ...
        await interaction.response.send_message(
            "あぶない！このコマンドはサーバーが爆発します！<@!712105359673917480> を呼んでね"
        )


@tree.command(name="status", description="通話ステータスを変更するよ")
@app_commands.describe(
    color="1:赤 2:黄 3:青", input_status="後ろの説明書きを入力してね"
)
async def test(
    interaction: discord.Interaction, color: int = -1, input_status: str = ""
):
    ...
    await statusChannel.edit(
        name=status, reason="status changed by " + interaction.user.name
    )
...
async def test(interaction: discord.Interaction):
    ...
    if interaction.user.voice == None:
        ...
        if interaction.user.voice.channel.permissions_for(interaction.user).connect:
            ...
            await interaction.response.send_message(
                interaction.user.voice.channel.name + "をロックしました",
                ephemeral=False,
            )
            return
        else:
            ...
            await interaction.response.send_message(
                interaction.user.voice.channel.name + "のロックを解除しました",
                ephemeral=False,
            )
            return
...
async def test(interaction: discord.Interaction, char: str):
    ...
    for i in range(len(char)):
        if char[i].isalpha() == False:
            await interaction.response.send_message(
                "デコードできません！", ephemeral=True
            )
            return
    await interaction.response.send_message(
        "その文字をデコードすると" + str(bingo.decode(char)) + "です！"
    )
